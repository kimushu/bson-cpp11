...
TEST(writer, add_string)
{
  ...
  ASSERT_BINEQ(
    "19 00 00 00 "
    "61 00 02 02 00 00 00 41 00 "
    "62 00 02 04 00 00 00 42 00 40 00 "
    "00 aa",
    buffer
  );
}
...
{
  std::uint8_t buffer1[] = {0x08,0x00,0x00,0x00,0x41,0x00,0x06,0x00};
  std::uint8_t buffer2[] = {0x08,0x00,0x00,0x00,0x42,0x00,0x06,0x00};
...
{
  std::uint8_t buffer[] = {
    0x0b,0x00,0x00,0x00,0x41,0x00,0x06,0x42,0x00,0x0a,0x00,0xaa
  };
  bson::reader r(buffer, sizeof(buffer));
  auto i = r.begin();
  ASSERT_EQ(r.begin(), i++);
...
{
  std::uint8_t buffer[] = {
    0x09,0x00,0x00,0x00,0x41,0x00,0x06,0xaa,0x00
  };
  bson::reader r(buffer, sizeof(buffer));
  auto i = r.begin();
  ASSERT_FALSE(i.fail());
...
TEST(reader, element_document)
{
  std::uint8_t buffer[] = {
    0x10,0x00,0x00,0x00,
      0x45,0x00,0x03,
        0x08,0x00,0x00,0x00,0x61,0x00,0x06,0x00,
    0x00,
  };
  bson::reader r(buffer, sizeof(buffer));
  auto i = r.begin();
...
TEST(reader, element_array)
{
  std::uint8_t buffer[] = {
    0x10,0x00,0x00,0x00,
      0x46,0x00,0x04,
        0x08,0x00,0x00,0x00,0x31,0x00,0x06,0x00,
    0x00,
  };
  bson::reader r(buffer, sizeof(buffer));
  auto i = r.begin();
...
TEST(reader, element_int32)
{
  std::uint8_t buffer[] = {
    0x0f,0x00,0x00,0x00,
      0x4f,0x00,0x10,0xef,0xbe,0xad,0xde,
      0x50,0x00,0x06,
    0x00,
  };
  bson::reader r(buffer, sizeof(buffer));
...
TEST(reader, element_int64)
{
  std::uint8_t buffer[] = {
    0x0f,0x00,0x00,0x00,
      0x51,0x00,0x12,0xef,0xbe,0xad,0xde,0xfe,0xca,0xad,0xba,
      0x52,0x00,0x06,
    0x00,
  };
  bson::reader r(buffer, sizeof(buffer));
...
TEST(reader, element_truthy)
{
  std::uint8_t buffer[] = {
    0x47,0x00,0x00,0x00,
      0x41,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0xf0,0x3f, // Non-zero double
      0x42,0x00,0x02,0x02,0x00,0x00,0x00,0x00,0x00,           // Non-empty string
      0x43,0x00,0x03,0x05,0x00,0x00,0x00,0x00,                // Empty document
      0x44,0x00,0x04,0x05,0x00,0x00,0x00,0x00,                // Empty array
      0x45,0x00,0x05,0x00,0x00,0x00,0x00,0x00,                // Empty binary
      0x46,0x00,0x08,0x01,                                    // true
      0x47,0x00,0x10,0x01,0x00,0x00,0x00,                     // Non-zero int32
      0x48,0x00,0x12,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00, // Non-zero int64
    0x00,
  };
  bson::reader r(buffer, sizeof(buffer));
  for (auto e : r) {
...
TEST(reader, element_falsy)
{
  std::uint8_t buffer[] = {
    0x4a,0x00,0x00,0x00,
      0x41,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00, // Zero double
      0x42,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0xf8,0x7f, // qNaN double
      0x43,0x00,0x01,0x01,0x00,0x00,0x00,0x00,0x00,0xf0,0x7f, // sNaN double
      0x44,0x00,0x02,0x01,0x00,0x00,0x00,0x00,                // Empty string
      0x45,0x00,0x06,                                         // undefined
      0x46,0x00,0x08,0x00,                                    // false
      0x47,0x00,0x06,                                         // null
      0x48,0x00,0x10,0x00,0x00,0x00,0x00,                     // Zero int32
      0x49,0x00,0x12,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00, // Zero int64
    0x00,
  };
  bson::reader r(buffer, sizeof(buffer));
  for (auto e : r) {
